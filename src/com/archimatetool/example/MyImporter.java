...
public class MyImporter implements IModelImporter {
    ...
    String MY_EXTENSION_WILDCARD = "*.csv"; //$NON-NLS-1$
    ...
    public void doImport() throws IOException {
        ...
        List<String> viewElements = new ArrayList<String>();
        // Views
        String[] views = {
                // Name, ID
                "A View", "view1"
       ...
       // String[] viewElements;
        ...
        try (CSVReader csvReader = new CSVReader(new FileReader(file.getPath()))) {
            ...
            while ((values = csvReader.readNext()) != null) {
            	if(values[2].trim().toLowerCase().equals("box")) {
            		elements.add("BusinessFunction");            		
            		elements.add(values[0].toString());
            		elements.add("elementId" + String.valueOf(i));
            		double height = Double.valueOf(values[1].replace(" in.", ""));
            		int pixels = 96;
            		int h = (int)(height*pixels);
            		double width = Double.valueOf(values[5].replace(" in.", ""));
            		int w = (int)(width*pixels);
            		double xl = Double.valueOf(values[6].replace(" in.", ""));
            		int x = (int)(xl*pixels);
            		double yl = Double.valueOf(values[7].replace(" in.", ""));
            		int y = (int)(yl*pixels);
            		viewElements.add("view1");
            		viewElements.add("elementId" + String.valueOf(i));
            		viewElements.add(String.valueOf(x));
            		viewElements.add(String.valueOf(-y));
            		viewElements.add(String.valueOf(w));
            		
            		viewElements.add(String.valueOf(h));
            		i++;
            	}
        ...
        //        "AssignmentRelationship", "", "relID2", "elementID1", "elementID3",
        //        "AssignmentRelationship", "", "relID3", "elementID2", "elementID3"
        //};
		...
		//	IArchimateRelationship relationship = createAndAddArchimateRelationship(model,
		//			(EClass) IArchimatePackage.eINSTANCE.getEClassifier(type), name, id);
        //    
        ...
        for(int i = 0; i < viewElements.size();) {
            String viewID = viewElements.get(i++);
            String refID = viewElements.get(i++);
            int x = Integer.parseInt(viewElements.get(i++));
            int y = Integer.parseInt(viewElements.get(i++));
            int width = Integer.parseInt(viewElements.get(i++));
            int height = Integer.parseInt(viewElements.get(i++));
